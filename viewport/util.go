...
// clampValZeroToMax clamps v to be between 0 and maximum inclusive
...
func getBytesLeftOfWidth(nBytes int, buffers []SingleItem, startBufferIdx int, widthToLeft int) string {
	if nBytes < 0 {
...
func getBytesRightOfWidth(nBytes int, buffers []SingleItem, endBufferIdx int, widthToRight int) string {
	if nBytes < 0 {
		panic("nBytes must be greater than 0")
	}
